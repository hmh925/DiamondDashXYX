package laya.webgl.canvas {
	...
	public class WebGLContext2D extends Context {
		...
		*/
		...
		override public function drawCanvas(canvas:HTMLCanvas, x:Number, y:Number, width:Number, height:Number):void {
			...
			if (src._targets) {
				...
				*/
				...
				if ((src as Object).touches) {
					((src as Object).touches as Array).forEach(function(v:CharRenderInfo):void { v.touch(); } );
				}
								
				submit = SubmitCanvas.create(src, _shader2D.ALPHA, _shader2D.filters);
			...
			if( _curMat._bTransform ){
				...
				if ( m.tx == 0 && m.ty == 0 && !dx && !dy) { }
					...
					for (i = 0, sz = points.length / 2; i < sz; i++) {
						var x2:Number = points[ci]+dx, y2:Number = points[ci + 1]+dy;
						points[ci] = x2 + m.tx;
						points[ci + 1] = y2+m.ty;
						ci += 2;
					}
		...
		public function fill():void {
			...
			for ( var i:int = 0, sz:int = tPath.paths.length; i < sz; i++) {
				...
				if (p.convex) { //convex的ib比较容易
					...
					idx = Earcut.earcut(p.path, null, 2);	//返回索引
					if (curvert > 0) {
						//修改ib
				...
				//填充mesh
				_pathMesh.addVertAndIBToMesh(this, p.path, rgba, idx);
		...
		public override function moveTo(x:Number, y:Number, b:Boolean = true):void {
			var tPath:Path = _getPath();
			tPath.newPath();
			tPath._lastOriX = x;
			tPath._lastOriY = y;
		 ...
		 * @param	b 是否应用矩阵
		 */
		public override function lineTo(x:Number, y:Number, b:Boolean = true):void {
			var tPath:Path = _getPath();
