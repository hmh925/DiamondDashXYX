package laya.d3.physics {
	...
	public class PhysicsComponent extends Component {
		...
		public function _derivePhysicsTransformation(physicTransformOut:*, force:Boolean):void {
			if (force || _getTransformFlag(Transform3D.TRANSFORM_WORLDMATRIX)) {
				...
				if (force || _getTransformFlag(Transform3D.TRANSFORM_WORLDSCALE)) {
					_colliderShape._setScale(transform.scale);
		...
		public function _updateTransformComponent(physicsTransform:*):void {
			var shapeOffsetE:Float32Array = _colliderShape.localOffset.elements;
			var shapeRotationE:Float32Array = _colliderShape.localRotation.elements;
			...
			if (shapeOffsetE[0] !== 0 || shapeOffsetE[1] !== 0 || shapeOffsetE[2] !== 0) {
				...
				physicVector3TransformQuat(_colliderShape.localOffset, rotationE[0], rotationE[1], rotationE[2], rotationE[3], rotShapePosition);
			...
			if (shapeRotationE[0] !== 0 || shapeRotationE[1] !== 0 || shapeRotationE[2] !== 0 || shapeRotationE[3] !== 1) {
				...
				_colliderShape.localRotation.invert(invertShapeRotaion);
				physicQuaternionMultiply(-nativeRotation.x(), nativeRotation.y(), nativeRotation.z(), -nativeRotation.w(), invertShapeRotaion, rotation);
			} else {
				rotationE[0] = -nativeRotation.x();
				rotationE[1] = nativeRotation.y();
				rotationE[2] = nativeRotation.z();
				rotationE[3] = -nativeRotation.w();
			}
